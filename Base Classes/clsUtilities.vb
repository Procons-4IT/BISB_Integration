...
#Region "BisB Integration"
    ...
    Dim testDomainName As String
    Dim testServerName As String
    ...
    Public Function SqlConnectionString() As String
        ...
        If oRec.RecordCount > 0 Then
            ...
            testuserName = uid
            testDbName = dbname
            testServerName = sername
            testDomainName = "bisbonline"
            ...
            ConnectionString = "data source=" & sername & ";Integrated Security=true;database=" & dbname & "; User id=" & uid & "; password=" & pwd
            ' ConnectionString = "data source=" & sername & ";Trusted_Connection=Yes;database=" & dbname & "; User id=" & uid '& "; password=" & pwd
            '  ConnectionString = "Data Source=" & sername & "; Trusted_Connection=Yes;Database=" & dbname
        End If
    ...
    Public Function getCheckNumber(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        'Dim connString As String = SqlConnectionString()

        'Dim returnValue As Boolean = LogonUser(testuserName, testDomainName, testPwd, LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_DEFAULT, safeTokenHandle)
        ...
        'End Using
        Dim connString As String = SqlConnectionString()
        Using New Impersonator(testuserName, testDomainName, testPwd)
            Dim oMatrix As SAPbouiCOM.Matrix
            ...
            Try
                ...
                If oTest.RecordCount > 0 Then
                    ...
                    If ds1.Tables(0).Rows.Count > 0 Then
                        ...
                        stQuery = "exec dbo.Insert_Stock_Move N'CO', -1, " & strChequeNo & ", 1, N'" & dblSPReturnValue & "', 911, " & userid & ", 0, 5, 6, '',-1"
                        con.Close()
                        con.Open()
                        ...
                        If 1 = 1 Then ' ds2.Tables(0).Rows.Count > 0 Then
                            ...
                            strCommand = "exec dbo.InsertIntoCashierOrderCheques 1, 1, 911, N'" & strChequeNo & "'," & dblCheckSum & ", N'IssueCOAgainstGL-SAP', N'" & strCurrency & "'," & dblCheckSum & ", N'" & strComments & "', N'" & dtDate.ToString("yyyy/MM/dd") & "', N'" & strCardName & "', '', '', '', '', 0, N'" & strBPAccount & "','BHD',1,''"
    ...
    Public Function ExportJournalEntries_Testing_Payroll(ByVal aPath As String, ByVal aCode As String, ByVal aPostingDate As Date) As Boolean
        ...
        For intLoop As Integer = 0 To oMainRec.RecordCount - 1
            ...
            Dim oDoc1, oDoc2 As New XmlDocument
            If strPaht <> "" Then
                If IO.Directory.Exists(strPaht) Then
                    ...
                    End If
                    If IO.Directory.Exists(strPaht & "\IFXResponse") = False Then
                        IO.Directory.CreateDirectory(strPaht & "\IFXResponse")
                    End If
            ...
            Dim doc As New XmlDocument
            doc.LoadXml(IFXResponse)
            oDoc2.LoadXml(IFXResponse)
            Try
                'Dim x As String = doc.SelectSingleNode("IFX/BankScvRs/RqUID)").InnerText.ToString
                Dim locx, locy, locy1 As String
                oDoc2.Save(strPaht & "\IFXResponse\JE_" & aPath & "IFXResponse.xml")
                locx = (doc.SelectSingleNode("IFX/BankSvcRs/FinancialMessageAddRs/Status/StatusCode").InnerText).ToString
                locy = (doc.SelectSingleNode("IFX/BankSvcRs/FinancialMessageAddRs/Status/Severity").InnerText).ToString
                locy1 = (doc.SelectSingleNode("IFX/BankSvcRs/FinancialMessageAddRs/Status/StatusDesc").InnerText).ToString
                If locx = "0" Then
    ...
    Public Function ExportJournalEntries_Testing_Payroll_Empployee(ByVal aPath As String, ByVal aCode As String, ByVal aPostingDate As Date) As Boolean
        ...
        For intLoop As Integer = 1 To 1 'oMainRec.RecordCount - 1
            ...
            If oRecItem.Fields.Item("U_Z_IFXPosting").Value = "C" Then
                ...
                writer.WriteStartElement("SignonRq")
                writer.WriteStartElement("RqUID")
                st = "BisB-Payroll-" & dtJEDate.ToString("yyyy-MM-dd")
                ...
                st = aPostingDate.ToString("yyyy-MM-dd")
                ...
                st = MonthName(dtJEDate.Month) & " Salary  " & Year(dtJEDate).ToString  ' dtJEDate.ToString("yyyy-MM-dd")
                writer.WriteString(st)
                ...
                Try
                    oDoc2.LoadXml(IFXResponse)
                    oDoc2.Save(strPaht & "\IFXResponse\JEEmp_" & aPath & "IFXResponse.xml")
                    'Dim x As String = doc.SelectSingleNode("IFX/BankScvRs/RqUID)").InnerText.ToString
                    Dim locx, locy, locy1 As String
                    locx = (doc.SelectSingleNode("IFX/BankSvcRs/FinancialMessageAddRs/Status/StatusCode").InnerText).ToString
                    locy = (doc.SelectSingleNode("IFX/BankSvcRs/FinancialMessageAddRs/Status/Severity").InnerText).ToString
    ...
    Public Function ExportJournalEntries_Testing_Payroll_Empployee_Debit(ByVal aBranch As String, ByVal aDept As String, ByVal aMonth As String, ByVal aYear As String, ByVal aPostingdate As Date) As Boolean
        Dim oRecItem, oRecItemCode, oTemp, oMainRec, oTemp1 As SAPbobsCOM.Recordset
        Dim Branch, Depart As String
        Dim strSQL, strFilename, strUserID As String
        Dim sValue As String
        Dim sPath, strLogDirectory, strPath, strMessage, strSelectedFolderPath, strExportFilePaty As String
        Dim blnErrorflag As Boolean
        ...
        For intLoop As Integer = 1 To 1 'oMainRec.RecordCount - 1
            ...
            ' strSQL = "SELECT sum(T0.[U_Z_NetSalary]),T0.[U_Z_Branch], T0.[U_Z_Dept] FROM [dbo].[@Z_PAYROLL1]  T0 where isnull(T0.U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and T0.U_Z_Month=" & aMonth & " and T0.U_Z_Year=" & aYear & " and  T0.Code in (" & strCode1 & ") group by T0.[U_Z_Branch], T0.[U_Z_Dept]"
            '  strSQL = "SELECT sum(T0.[U_Z_NetSalary]) 'U_Z_NetSalary',T0.[U_Z_Branch], T0.[U_Z_Dept],T0.[U_Z_PayDate] FROM [dbo].[@Z_PAYROLL1]  T0 where isnull(T0.U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and isnull(U_Z_IFXDebit,'P')='P' and  T0.U_Z_Month=" & aMonth & " and T0.U_Z_Year=" & aYear & " and  T0.U_Z_Branch ='" & aBranch & "' and U_Z_Dept='" & aDept & "' group by T0.[U_Z_Branch], T0.[U_Z_Dept],T0.[U_Z_PayDate]"
            strSQL = "SELECT sum(T0.[U_Z_NetSalary]) 'U_Z_NetSalary' FROM [dbo].[@Z_PAYROLL1]  T0 where isnull(T0.U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and isnull(U_Z_IFXDebit,'P')='P' and  T0.U_Z_Month=" & aMonth & " and T0.U_Z_Year=" & aYear ' & " and  T0.U_Z_Branch ='" & aBranch & "' and U_Z_Dept='" & aDept & "'" ' group by T0.[U_Z_Branch], T0.[U_Z_Dept],T0.[U_Z_PayDate]"
            oRecItemCode = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRecItem.DoQuery(strSQL)
            Dim strTRGFileName As String
            Dim dtDateTime As String
            ...
            If 1 = 1 Then 'oRecItem.Fields.Item("U_Z_IFXPosting").Value = "C" Then
                ...
                strSQL = " SELECT  max(U_Z_PayDate) 'U_Z_PayDate'  FROM [dbo].[@Z_PAYROLL1]  T0 where isnull(T0.U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and isnull(U_Z_IFXDebit,'P')='P' and  T0.U_Z_Month=" & aMonth & " and T0.U_Z_Year=" & aYear '& " and  T0.U_Z_Branch ='" & aBranch & "' and U_Z_Dept='" & aDept & "' group by T0.[U_Z_Branch], T0.[U_Z_Dept],T0.[U_Z_PayDate]"
                oTemp1.DoQuery(strSQL)

                dtJEDate = oTemp1.Fields.Item("U_Z_PayDate").Value
                dtDateTime = aPostingdate.ToString("yyyy-MM-dd") ' dtJEDate.ToString("yyyy-MM-dd")
                strTRGFileName = strExportFilePaty & "\JE_" & FILedatetiem & ".trg"
                strFilename = strExportFilePaty & "\JE_" & FILedatetiem & ".xml"
                '(strFilename, System.Text.Encoding.UTF8)
                Dim st As String
                ...
                straccount = "01-01"
                writer.WriteStartElement("AcctId")
                oRecItemCode.DoQuery("Select * from OACT where FormatCode='" & strDebitAccount & "'")
                If oRecItemCode.Fields.Item("U_Z_IFXBranch").Value <> "" Then
                    aBranch = oRecItemCode.Fields.Item("U_Z_IFXBranch").Value
                End If
                If oRecItemCode.Fields.Item("U_Z_IFXDept").Value <> "" Then
                    aDept = oRecItemCode.Fields.Item("U_Z_IFXDept").Value
                End If
                ...
                writer.WriteStartElement("EffDt")
                st = aPostingdate.ToString("yyyy-MM-dd") ' dtJEDate.ToString("yyyy-MM-dd")
                writer.WriteString(st)
                writer.WriteEndElement() 'EffDt
                writer.WriteStartElement("ApplType")
                st = "GL"
                ...
                MyXMLString = MyXMLString.Replace("utf-16", "utf-8")
                MyXMLString = MyXMLString.Replace("standalone=""yes""", "")
                If strPaht <> "" Then
                    If IO.Directory.Exists(strPaht) Then
                        oDoc1.LoadXml(MyXMLString)
                        blnFileExists = True
                ...
                doc.LoadXml(IFXResponse)
                Try
                    Dim oDoc2 As New XmlDocument
                    Dim aPath As String = Branch & "_" & Depart
                    oDoc2.LoadXml(IFXResponse)
                    oDoc2.Save(strPaht & "\IFXResponse\JEEmp_" & aPath & "IFXResponse.xml")
                    'Dim x As String = doc.SelectSingleNode("IFX/BankScvRs/RqUID)").InnerText.ToString
                    Dim locx, locy, locy1 As String
                    ...
                    If locx = "0" Then
                        If blnFileExists Then
                            ...
                            If File.Exists(strPaht & "\Success\JE_EmpDebit_" & Branch & "_" & Depart & "_Success.xml") Then
                                File.Delete(strPaht & "\Success\JE_EmpDebit_" & Branch & "_" & Depart & "_Success.xml")
                            End If
                            oDoc1.Save(strPaht & "\Success\JE_EmpDebit_" & Branch & "_" & Depart & "_Success.xml")
                        End If
                        '  strSQL = "Update [@Z_PAYROLL1] set U_Z_IFXDebit='C'  where isnull(U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and isnull(U_Z_IFXDebit,'P')='P' and U_Z_PayDate='" & dtJEDate.ToString("yyyy-MM-dd") & "' and  U_Z_Month=" & aMonth & " and U_Z_Year=" & aYear & " and  U_Z_Branch ='" & Branch & "' and U_Z_Dept='" & Depart & "'"
                        strSQL = "Update [@Z_PAYROLL1] set U_Z_IFXDebit='C'  where isnull(U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and isnull(U_Z_IFXDebit,'P')='P' and  U_Z_Month=" & aMonth & " and U_Z_Year=" & aYear ' & " and  U_Z_Branch ='" & Branch & "' and U_Z_Dept='" & Depart & "'"
                        oRecItem.DoQuery(strSQL)
                        Return True
                    Else
                        strMessage = "Error while transfering Journal Entry No : " & strJNo & " : Error : " & locy1
                        ...
                        ' oRecItem.DoQuery("Update [@Z_PAYROLL1] set U_Z_IFXPAY='P',U_Z_IFXEmpRes='" & locy1 & "' where Code='" & aPath & "' and isnull(U_Z_IFXPosting,'P')='C'")
                        If blnFileExists = True Then
                            If File.Exists(strPaht & "\Error\JE_EmpDebit_" & Branch & "_" & Depart & "_Error.xml") Then
                                File.Delete(strPaht & "\Error\JE_EmpDebit_" & Branch & "_" & Depart & "_Error.xml")
                            End If
                            oDoc1.Save(strPaht & "\Error\JE_EmpDebit_" & Branch & "_" & Depart & "_Error.xml")
                        End If
                    End If
                    Return False
                    'WriteErrorlog(strMessage, strErrorFileName)
        ...
        'End If
    End Function
    Public Function ExportJournalEntries_Testing_Payroll_Empployee_Debit_Latest(ByVal aBranch As String, ByVal aDept As String, ByVal aMonth As String, ByVal aYear As String, ByVal aPostingdate As Date) As Boolean
        Dim oRecItem, oRecItemCode, oTemp, oMainRec As SAPbobsCOM.Recordset
        Dim Branch, Depart As String
        Dim strSQL, strFilename, strUserID As String
        Dim sValue As String
                ...
                Try
                    ...
                    If locx = "0" Then
                        If blnFileExists Then
                            ...
                            oDoc1.Save(strPaht & "\Success\JE_EmpDebit_" & Branch & "_" & Depart & "_Success.xml")
                        End If
                        strSQL = "Update [@Z_PAYROLL1] set U_Z_IFXDebit='P' [@Z_PAYROLL1]   where isnull(U_Z_JVNo,'')<>'' and Isnull(U_Z_IFXPosting,'P')='C' and isnull(U_Z_IFXPAY,'P')='C' and isnull(U_Z_IFXDebit,'P')='P' and  U_Z_Month=" & aMonth & " and U_Z_Year=" & aYear & " and  U_Z_Branch ='" & aBranch & "' and U_Z_Dept='" & aDept & "'"
                        oRecItem.DoQuery(strSQL)
                        Return True
                    Else
                        strMessage = "Error while transfering Journal Entry No : " & strJNo & " : Error : " & locy1
        ...
        'End If
    End Function
    Public Function ExportJournalEntries_Testing_Payroll_Empployee_Debit_Old(ByVal aBranch As String, ByVal aDept As String, ByVal aMonth As String, ByVal aYear As String, ByVal aPostingdate As Date) As Boolean
        Dim oRecItem, oRecItemCode, oTemp, oMainRec As SAPbobsCOM.Recordset
        Dim Branch, Depart As String
        Dim strSQL, strFilename, strUserID As String
        Dim sValue As String
    ...
    Private Function SendXMLtoIFX(ByVal aString As String) As String
        ...
        otest.DoQuery("Select * from [@Z_IFXSetup]")
        If otest.RecordCount > 0 Then
            MyService1.Url = otest.Fields.Item("U_Z_URL").Value ' for reference "http://10.10.151.52:7001/UBSWebservice/UBSWebservice.jws?WSDL"
            netCred.UserName = otest.Fields.Item("U_Z_UID").Value ' for reference "FluidUser"
            netCred.Password = otest.Fields.Item("U_Z_PWD").Value  'for reference "FluidUser"
